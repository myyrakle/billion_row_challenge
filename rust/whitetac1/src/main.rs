...
fn solution(path: &str) -> String {
    ...
    let maps: Arc<Mutex<Vec<Box<IntMap<Status>>>>> =
    ...
    for chunks_start_index in 0..chunks_start_indexes.len() - 1 {
        ...
        buffers.push(mmap[chunk_start..chunk_end].as_ref());
    }
    buffers.par_iter().for_each(|full_buffer| {
        let mut map1: IntMap<Status> = IntMap::new();
        for buffer in full_buffer.split(|c| *c == b'\n') {
            ...
            if map1.contains_key(city_code) {
                let status = map1.get_mut(city_code).unwrap();
        ...
        maps.lock().unwrap().push(Box::new(map1));
        std::mem::forget(full_buffer);
    });
    ...
    for m in unlock_maps.iter() {
        for (city_code, status) in m.iter() {
            if map.contains_key(*city_code) {
                let status2: &mut Status = map.get_mut(*city_code).unwrap();
